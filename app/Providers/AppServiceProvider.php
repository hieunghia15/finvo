...
class AppServiceProvider extends ServiceProvider
{
    ...
    protected function configureRateLimiting(): void
    {
        RateLimiter::for('login', fn (Request $request) => Limit::perMinute(10)
            ->by($request->ip())
            ->response($this->throttledResponse(__('auth.throttle'))));

        RateLimiter::for('register', fn (Request $request) => Limit::perMinute(5)
            ->by($request->ip())
            ->response($this->throttledResponse(__('Too many registration attempts. Please try again in :seconds seconds.'))));
        ...
        RateLimiter::for('password-update', fn (Request $request) => Limit::perMinute(5)
            ...
            ->response($this->throttledResponse(
                __('Too many password change attempts. Please try again in :seconds seconds.'),
                'current_password',
            )));
    }
     ...
     *
     * @param  string  $message  Already translated message; may use :seconds and :minutes.
